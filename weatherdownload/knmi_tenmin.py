...
from .knmi_parser import KNMI_NORMALIZED_SUBDAILY_COLUMNS, parse_knmi_tenmin_netcdf_bytes
from .knmi_registry import KNMI_TENMIN_FILENAME_PREFIX, get_dataset_spec
from .queries import ObservationQuery

_KNMI_TENMIN_FILENAME_PATTERN = re.compile(r'KMDS__OPER_P___10M_OBS_L2_(?P<stamp>\d{12})\.nc$', re.IGNORECASE)


def download_tenmin_observations_knmi(
    query: ObservationQuery,
    timeout: int = 60,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    if query.dataset_scope != 'historical' or query.resolution != '10min':
        raise UnsupportedQueryError('The KNMI 10-minute downloader only supports historical/10min.')
    if not query.elements:
        raise UnsupportedQueryError('The KNMI 10-minute downloader requires at least one element.')
    ...
    filenames = _list_tenmin_filenames(request_start=request_start, request_end=request_end, timeout=timeout, api_key=api_key)
    if not filenames:
        raise EmptyResultError('No KNMI 10-minute files were found for the requested timestamp range.')
    ...
    for filename in filenames:
        ...
        parsed_payloads.append(parse_knmi_tenmin_netcdf_bytes(netcdf_bytes))

    normalized_frames = [
        normalize_tenmin_observations_knmi(payload, query, station_metadata=metadata_table)
...
def normalize_tenmin_observations_knmi(
...
def _list_tenmin_filenames(*, request_start: pd.Timestamp, request_end: pd.Timestamp, timeout: int, api_key: str) -> list[str]:
    spec = get_dataset_spec('historical', '10min')
    begin_filename = f'{KNMI_TENMIN_FILENAME_PREFIX}{request_start.strftime("%Y%m%d%H%M")}.nc'
    ...
    while True:
        ...
        if not isinstance(files, list):
            raise DownloadError('KNMI 10-minute file listing returned an invalid files payload.')
        for file_info in files:
            filename = file_info.get('filename') if isinstance(file_info, dict) else None
            if not isinstance(filename, str):
                continue
            file_timestamp = _parse_knmi_tenmin_filename_timestamp(filename)
...
def _parse_knmi_tenmin_filename_timestamp(filename: str) -> pd.Timestamp | None:
    match = _KNMI_TENMIN_FILENAME_PATTERN.search(filename.strip())
    if match is None:
        return None
    return pd.Timestamp(match.group('stamp')).tz_localize('UTC')
