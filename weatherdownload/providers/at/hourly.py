...
GEOSPHERE_HOURLY_API_URL = 'https://dataset.api.hub.geosphere.at/v1/station/historical/klima-v2-1h'
_HOURLY_SOLAR_RADIATION_SCALE = 0.0036


def download_hourly_observations_geosphere(
    ...
    csv_text = _download_hourly_csv(query, request_start=request_start, request_end=request_end, timeout=timeout)
    parsed = parse_geosphere_hourly_csv(csv_text)
    normalized = normalize_hourly_observations_geosphere(parsed, query, station_metadata=metadata_table)
    if normalized.empty:
...
def _download_hourly_csv(query: ObservationQuery, *, request_start: str, request_end: str, timeout: int) -> str:
    ...
    response = requests.get(GEOSPHERE_HOURLY_API_URL, params=params, timeout=timeout)
    response.raise_for_status()
    response.encoding = 'utf-8'
    return response.text


def normalize_hourly_observations_geosphere(
    table: pd.DataFrame,
    query: ObservationQuery,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    ...
    for raw_code in query.elements or []:
        ...
        if raw_code == 'cglo':
            values = values * _HOURLY_SOLAR_RADIATION_SCALE
