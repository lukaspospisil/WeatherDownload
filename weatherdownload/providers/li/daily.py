...
from .metadata import read_station_metadata_li
...
class LiDailyDownloadTarget:
    station_id: str
    asset_url: str
    source_kind: str


def download_daily_observations_li(
    query: ObservationQuery,
    timeout: int = 60,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    if query.provider != 'meteoswiss' or query.resolution != 'daily':
        raise UnsupportedQueryError('The MeteoSwiss Liechtenstein daily downloader only supports meteoswiss/daily.')
    if not query.elements:
        raise UnsupportedQueryError('The MeteoSwiss Liechtenstein daily downloader requires at least one element.')

    metadata_table = station_metadata if station_metadata is not None else read_station_metadata_li(timeout=timeout)
    if metadata_table.empty:
        raise EmptyResultError('No MeteoSwiss Liechtenstein station metadata are available.')
    ...
    if missing_station_ids:
        raise StationNotFoundError(f"No MeteoSwiss Liechtenstein station metadata found for station_id: {', '.join(missing_station_ids)}")

    targets = build_li_daily_download_targets(query, timeout=timeout)
    ...
    if not parsed_tables:
        raise StationNotFoundError(f"No MeteoSwiss Liechtenstein daily data found for station_id: {', '.join(sorted(query.station_ids))}")

    normalized = normalize_daily_observations_li(pd.concat(parsed_tables, ignore_index=True), query)
    if normalized.empty:
        raise EmptyResultError('No observations found for the given query.')
    return normalized.loc[:, CH_NORMALIZED_DAILY_COLUMNS]


def build_li_daily_download_targets(query: ObservationQuery, timeout: int = 60) -> list[LiDailyDownloadTarget]:
    spec = get_dataset_spec(query.provider, query.resolution)
    if not spec.implemented:
        raise UnsupportedQueryError('The requested MeteoSwiss Liechtenstein dataset path is not implemented.')

    current_year = current_year_utc()
    request_start = pd.Timestamp(query.start_date)
    request_end = pd.Timestamp(query.end_date)
    targets: list[LiDailyDownloadTarget] = []
    for station_id in query.station_ids:
        ...
        if historical_url is not None and (query.all_history or request_start.year < current_year):
            targets.append(LiDailyDownloadTarget(station_id=station_id, asset_url=historical_url, source_kind='historical'))
        if recent_url is not None and (query.all_history or request_end.year >= current_year):
            targets.append(LiDailyDownloadTarget(station_id=station_id, asset_url=recent_url, source_kind='recent'))
    return targets


def normalize_daily_observations_li(table: pd.DataFrame, query: ObservationQuery) -> pd.DataFrame:
    if table.empty:
