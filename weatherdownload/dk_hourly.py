...
from .dk_registry import DMI_CLIMATE_STATION_VALUE_URL
...
def download_hourly_observations_dk(
    query: ObservationQuery,
    timeout: int = 60,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    if query.dataset_scope != 'historical' or query.resolution != '1hour':
        raise UnsupportedQueryError('The DMI Denmark hourly downloader only supports historical/1hour.')
    if not query.elements:
        raise UnsupportedQueryError('The DMI Denmark hourly downloader requires at least one element.')
    ...
    for station_id in query.station_ids:
        for parameter_id in query.elements or []:
            payload = _download_hourly_payload(
            ...
            normalized = normalize_hourly_observations_dk(payload, query, station_metadata=metadata_table)
...
def normalize_hourly_observations_dk(
    ...
    for feature in features:
        ...
        if raw_code not in (query.elements or []):
            continue
        timestamp = observation_timestamp_from_interval_end(properties.get('to'))
...
def _download_hourly_payload(*, station_id: str, parameter_id: str, request_start: pd.Timestamp, request_end: pd.Timestamp, timeout: int) -> dict[str, object]:
    params = {
