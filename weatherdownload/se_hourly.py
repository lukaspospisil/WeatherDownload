...
from .se_metadata import read_station_metadata_se
from .se_parser import SE_NORMALIZED_SUBDAILY_COLUMNS, parse_se_hourly_csv
from .se_registry import SMHI_METOBS_API_BASE


def download_hourly_observations_se(
    ...
    for station_id in query.station_ids:
        for parameter_id in query.elements or []:
            csv_text = _download_hourly_csv(station_id=station_id, parameter_id=parameter_id, timeout=timeout)
            normalized = normalize_hourly_observations_se(
                parse_se_hourly_csv(csv_text),
                query,
    ...
    return combined.loc[:, SE_NORMALIZED_SUBDAILY_COLUMNS]


def normalize_hourly_observations_se(
    ...
    *,
    ...
    if station_id not in query.station_ids or parameter_id not in (query.elements or []):
        return pd.DataFrame(columns=SE_NORMALIZED_SUBDAILY_COLUMNS)
    ...
    for record in records.itertuples(index=False):
        if record.timestamp < request_start or record.timestamp > request_end:
            continue
        rows.append(
            {
    ...
    return combined.loc[:, SE_NORMALIZED_SUBDAILY_COLUMNS].reset_index(drop=True)
...
def _download_hourly_csv(*, station_id: str, parameter_id: str, timeout: int) -> str:
