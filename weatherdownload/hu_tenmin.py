...
from .hu_registry import HU_TENMIN_HISTORICAL_URL, HU_TENMIN_RECENT_URL, get_dataset_spec
from .queries import ObservationQuery

_HU_TENMIN_ARCHIVE_PATTERN = re.compile(
    r'HABP_10M_(?P<station_id>\d+)_(?P<start>\d{8})_(?P<end>\d{8})_hist\.zip'
)
_HU_TENMIN_PRODUCT_PATTERN = re.compile(
    r'HABP_10M_\d{8}_\d{8}_(?P<station_id>\d+)\.csv'
)


@dataclass(frozen=True)
class HuTenminDownloadTarget:
    station_id: str
    archive_url: str


def download_tenmin_observations_hu(
    query: ObservationQuery,
    timeout: int = 60,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    if query.dataset_scope != 'historical' or query.resolution != '10min':
        raise UnsupportedQueryError('The HungaroMet Hungary 10-minute downloader only supports historical/10min.')
    if not query.elements:
        raise UnsupportedQueryError('The HungaroMet Hungary 10-minute downloader requires at least one element.')
    ...
    targets = build_hu_tenmin_download_targets(query, timeout=timeout)
    ...
    for target in targets:
        try:
            ...
            continue
        parsed_tables.append(_parse_tenmin_archive(archive_bytes, target.station_id))

    if not parsed_tables:
        raise StationNotFoundError(f"No HungaroMet Hungary 10-minute data found for station_id: {', '.join(sorted(query.station_ids))}")

    normalized = normalize_tenmin_observations_hu(pd.concat(parsed_tables, ignore_index=True), query)
    if normalized.empty:
        raise EmptyResultError('No observations found for the given query.')
    return normalized.loc[:, HU_NORMALIZED_SUBDAILY_COLUMNS]


def build_hu_tenmin_download_targets(query: ObservationQuery, timeout: int = 60) -> list[HuTenminDownloadTarget]:
    ...
    historical_entries = parse_hu_directory_listing(listing_html, _HU_TENMIN_ARCHIVE_PATTERN)

    targets: list[HuTenminDownloadTarget] = []
    for station_id in query.station_ids:
        for entry in historical_entries:
            ...
            if archive_end < request_start or archive_start > request_end:
                continue
            archive_name = f"HABP_10M_{station_id}_{entry['start']}_{entry['end']}_hist.zip"
            targets.append(HuTenminDownloadTarget(station_id=station_id, archive_url=HU_TENMIN_HISTORICAL_URL + archive_name))
        if request_end >= pd.Timestamp(year=pd.Timestamp.today(tz='UTC').year, month=1, day=1, tz='UTC'):
            targets.append(
                HuTenminDownloadTarget(
                    station_id=station_id,
                    archive_url=f'{HU_TENMIN_RECENT_URL}HABP_10M_{station_id}_akt.zip',
                )
            )
    return targets


def normalize_tenmin_observations_hu(table: pd.DataFrame, query: ObservationQuery) -> pd.DataFrame:
    if table.empty:
...
def _fetch_historical_directory_listing(timeout: int) -> str:
    response = requests.get(HU_TENMIN_HISTORICAL_URL, timeout=timeout)
...
def _parse_tenmin_archive(archive_bytes: bytes, station_id: str) -> pd.DataFrame:
    with zipfile.ZipFile(io.BytesIO(archive_bytes)) as archive:
        product_names = [
            name for name in archive.namelist()
            if _HU_TENMIN_PRODUCT_PATTERN.fullmatch(name.split('/')[-1])
        ]
        if not product_names:
            raise ValueError(f'No HungaroMet 10-minute product file found in archive for station_id {station_id}.')
