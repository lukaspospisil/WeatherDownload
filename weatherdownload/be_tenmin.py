...
from .be_parser import BE_NORMALIZED_SUBDAILY_COLUMNS, normalize_be_station_id, parse_be_feature_collection_json
from .be_registry import RMI_AWS_TENMIN_LAYER, RMI_AWS_WFS_URL
...
def download_tenmin_observations_be(
    query: ObservationQuery,
    timeout: int = 60,
    station_metadata: pd.DataFrame | None = None,
) -> pd.DataFrame:
    if query.dataset_scope != 'historical' or query.resolution != '10min':
        raise UnsupportedQueryError('The RMI/KMI Belgium 10-minute downloader only supports historical/10min.')
    if not query.elements:
        raise UnsupportedQueryError('The RMI/KMI Belgium 10-minute downloader requires at least one element.')
    ...
    for station_id in query.station_ids:
        payloads.append(_download_tenmin_payload(station_id=station_id, request_start=request_start, request_end=request_end, timeout=timeout))

    normalized_frames = [normalize_tenmin_observations_be(payload, query, station_metadata=metadata_table) for payload in payloads]
...
def normalize_tenmin_observations_be(
...
def _download_tenmin_payload(*, station_id: str, request_start: pd.Timestamp, request_end: pd.Timestamp, timeout: int) -> dict[str, object]:
    params = {
        'service': 'WFS',
        'version': '1.0.0',
        'request': 'GetFeature',
        'typeName': RMI_AWS_TENMIN_LAYER,
