...
from .ch_metadata import read_station_metadata_ch
...
from .ch_registry import get_dataset_spec
...
def download_hourly_observations_ch(
    ...
    targets = build_ch_hourly_download_targets(query, timeout=timeout)
    parsed_tables: list[pd.DataFrame] = []
    for target in targets:
        table = _download_hourly_asset(target.asset_url, timeout=timeout)
        if table.empty:
            continue
    ...
    if not parsed_tables:
        raise StationNotFoundError(f"No MeteoSwiss Switzerland hourly data found for station_id: {', '.join(sorted(query.station_ids))}")

    normalized = normalize_hourly_observations_ch(pd.concat(parsed_tables, ignore_index=True), query)
    if normalized.empty:
...
def build_ch_hourly_download_targets(query: ObservationQuery, timeout: int = 60) -> list[ChHourlyDownloadTarget]:
    ...
    targets: list[ChHourlyDownloadTarget] = []
    for station_id in query.station_ids:
        ...
        for asset_name, asset_url in assets.items():
            if not asset_name.startswith(f'ogd-smn_{station_id.lower()}_h_historical_'):
                continue
            ...
            asset_end = pd.Timestamp(year=end_year, month=12, day=31, hour=23, tz='UTC')
            if asset_end < request_start or asset_start > request_end:
                continue
            targets.append(ChHourlyDownloadTarget(station_id=station_id, asset_url=asset_url, source_kind='historical'))
        recent_url = assets.get(f'ogd-smn_{station_id.lower()}_h_recent.csv')
        if recent_url is not None and (query.all_history or request_end.year >= current_year):
            targets.append(ChHourlyDownloadTarget(station_id=station_id, asset_url=recent_url, source_kind='recent'))
    return targets



def normalize_hourly_observations_ch(table: pd.DataFrame, query: ObservationQuery) -> pd.DataFrame:
    if table.empty:
...
def _download_hourly_asset(asset_url: str, timeout: int) -> pd.DataFrame:
