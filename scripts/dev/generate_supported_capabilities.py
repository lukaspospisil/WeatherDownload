...
SOURCE_DESCRIPTIONS: dict[tuple[str, str, str], str] = {
    ...
    ('CY', 'ghcnd', 'daily'): 'NOAA GHCN-Daily',
    ...
    ('LI', 'meteoswiss', 'daily'): 'MeteoSwiss A1 Liechtenstein daily station observations',
    ...
    ('LV', 'lvgmc', '1hour'): 'LVGMC recent meteorological archive hourly observations',
    ('LV', 'lvgmc', 'daily'): 'LVGMC recent meteorological archive observations aggregated to daily UTC-date values',
    ('MD', 'ghcnd', 'daily'): 'NOAA GHCN-Daily',
    ('ME', 'ghcnd', 'daily'): 'NOAA GHCN-Daily',
    ('MK', 'ghcnd', 'daily'): 'NOAA GHCN-Daily',
    ...
    ('RS', 'ghcnd', 'daily'): 'NOAA GHCN-Daily',
...
NOTES: dict[tuple[str, str, str], str] = {
    ...
    ('CY', 'ghcnd', 'daily'): 'Direct-prefix GHCN fallback for Cyprus only; raw GHCN station ids with prefix CY; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready.',
    ...
    ('LI', 'meteoswiss', 'daily'): 'Official MeteoSwiss A1 daily wrapper filtered to the Vaduz station (`VAD`) in Liechtenstein. Source-backed fields only: tas_mean, tas_max, tas_min, precipitation, wind_speed, wind_speed_max, relative_humidity, vapour_pressure, pressure, sunshine_duration, and solar_radiation. No provider-side FAO derivation, no open_water_evaporation, and no snow-depth claim from the automatic-station foehn index field.',
    ...
    ('LV', 'lvgmc', '1hour'): 'Official LVGMC recent meteorological archive hourly slice queried through the CKAN DataStore API. The public archive covers roughly the last 365 days rather than a long-term climatological hourly history. LVGMC timestamps are documented as period-end timestamps, so WeatherDownload exposes the represented hour as `timestamp = DATETIME - 1 hour`. The current observed hourly mapping exposes tas_mean, precipitation, wind_speed, wind_speed_max, relative_humidity, pressure, and snow_depth only; unsupported tas_max, tas_min, vapour_pressure, sunshine_duration, solar_radiation, wind_direction, and cloud_cover are not provider-derived.',
    ...
    ('MD', 'ghcnd', 'daily'): 'Direct-prefix GHCN fallback for Moldova only; raw GHCN station ids with prefix MD; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready.',
    ('ME', 'ghcnd', 'daily'): 'Mapped-prefix GHCN wrapper using raw GHCN station ids with prefix MJ; direct GHCND prefix ME is not used for Montenegro; inventory-driven station elements; no EVAP/open_water_evaporation.',
    ('MK', 'ghcnd', 'daily'): 'Direct-prefix GHCN fallback for North Macedonia only; raw GHCN station ids with prefix MK; NOAA\'s GHCND country list labels MK as "Macedonia"; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready.',
    ...
    ('RS', 'ghcnd', 'daily'): 'Mapped-prefix GHCN wrapper using raw GHCN station ids with prefix RI; NOAA GHCND prefix RS means Russia and is not used for Serbia; inventory-driven station elements; no EVAP/open_water_evaporation.',
