...
| `PL` | Stable | `historical`, `historical_klimat` | `daily` | `historical / daily`: `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `sunshine_duration`; `historical_klimat / daily`: `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | Official IMGW station list with source-backed 5-character station code and station name; coordinates, elevation, and validity range are not exposed by the implemented source file |
...
- raw IMGW daily status codes such as `WSTD`, `WSMDB`, and `WPKSN` stay in `flag`; normalized `quality` remains null
- no provider-side derivations or cross-resolution aggregations are added
