...
Veřejný query model používá čtyři dimenze:

- `country` vybírá zemi
- `provider` vybírá konkrétní datový zdroj nebo produkt v rámci dané země
- `resolution` vybírá časové rozlišení
- `element` vybírá canonical meteorologickou veličinu

Hodnoty `provider` jsou provider-specific a nejsou globálně standardizované mezi zeměmi. Například `CZ / historical_csv`, `SK / recent`, `PL / historical_klimat` a `US / ghcnd` označují různé zdrojové cesty.
...
- `--provider historical` je stále platný a vrací stejný výsledek
