...
| `CA` | `eccc` | `1hour` | `tas_mean`, `relative_humidity` | Environment and Climate Change Canada GeoMet hourly climate observations | Live GeoMet climate-hourly fetching with pagination; station_id is ECCC CLIMATE_IDENTIFIER. Station discovery is conservative because climate-hourly is a subset collection, and element coverage can vary by station/time. Hourly slice currently maps only tas_mean and relative_humidity. |
