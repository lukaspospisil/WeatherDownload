...
| --- | --- | --- | --- | --- | --- |
| `AL` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Direct-prefix GHCN fallback for Albania only; raw GHCN station ids with prefix AL; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready. |
...
| `BE` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily station observations | Direct-prefix ghcnd fallback for Belgium stations only. |
...
| `BE` | `rmi` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `wind_speed`, `wind_speed_max`, `relative_humidity`, `pressure`, `sunshine_duration` | RMI/KMI AWS daily observations | Provider-side daily aggregates; raw QC stays in flag; sunshine minutes normalized to hours. |
...
| `CY` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Direct-prefix GHCN fallback for Cyprus only; raw GHCN station ids with prefix CY; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready. |
...
| `LV` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Thin shared GHCN wrapper; raw GHCN station ids; inventory-driven station elements; no EVAP/open_water_evaporation. |
| `LV` | `lvgmc` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `wind_speed`, `wind_speed_max`, `relative_humidity`, `pressure`, `snow_depth` | LVGMC recent meteorological archive observations aggregated to daily UTC-date values | Official LVGMC recent meteorological archive slice queried through the CKAN DataStore API. WeatherDownload aggregates hourly AVG/MIN/MAX/SUM records into deterministic UTC-date daily values using the documented period-end rule `hourly_period_date = DATETIME - 1 hour`. This is not an official climatological daily summary product. The current observed mapping exposes tas_mean, tas_max, tas_min, precipitation, wind_speed, wind_speed_max, relative_humidity, pressure, and snow_depth only; unsupported vapour_pressure, sunshine_duration, solar_radiation, wind_direction, and cloud_cover are not provider-derived. `ghcnd` remains available as a separate daily provider. |
| `MD` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Direct-prefix GHCN fallback for Moldova only; raw GHCN station ids with prefix MD; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready. |
| `MK` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Direct-prefix GHCN fallback for North Macedonia only; raw GHCN station ids with prefix MK; NOAA's GHCND country list labels MK as "Macedonia"; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready. |
| `MT` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Direct-prefix GHCN fallback for Malta only; raw GHCN station ids with prefix MT; inventory-driven station elements; unsupported wind, humidity, pressure, vapour_pressure, sunshine_duration, and solar_radiation are not provider-derived; not FAO-ready. |
...
| `RS` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` | NOAA GHCN-Daily | Mapped-prefix GHCN wrapper using raw GHCN station ids with prefix RI; NOAA GHCND prefix RS means Russia and is not used for Serbia; inventory-driven station elements; no EVAP/open_water_evaporation. |
