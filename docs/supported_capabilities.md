...
| `ES` | `aemet` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `wind_speed`, `sunshine_duration` | AEMET OpenData daily climatological observations | Requires AEMET OpenData API key; observed-only daily slice; velmedia is converted from km/h to canonical m/s; trace precipitation `Ip` is mapped to 0.0 mm; not FAO-ready in the first pass. |
