...
| `LU` | `asta` | `daily` | `tas_mean`, `tas_max`, `tas_min` | ASTA daily weather measurements from the Luxembourg agrometeorological station network | Official ASTA daily WFS slice; current conservative first pass exposes only the clearly identified 2 m temperature layers via stable ASTA station ids such as AGM_022; other ASTA daily variables exist in the source but remain unmapped here pending clearer unit/semantic verification; observed-only and not FAO-ready. |
