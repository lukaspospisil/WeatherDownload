...
| `CA` | `eccc` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation` | Environment and Climate Change Canada GeoMet daily climate observations | Registered, but currently fixture/source_url-backed only; live GeoMet fetching and pagination are intentionally not implemented yet. `ghcnd` remains available as a separate daily provider. |
