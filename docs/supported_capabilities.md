...
| `LU` | `meteolux` | `daily` | `tas_max`, `tas_min`, `precipitation` | MeteoLux INSPIRE WFS daily weather measurements at Luxembourg Findel Airport | National MeteoLux daily WFS slice for Luxembourg/Findel Airport only; observed-only tas_max, tas_min, and precipitation; precipitation days are defined by the source as 06:00 UTC to 06:00 UTC of the following day; not FAO-ready and Rn/net radiation is not downloaded. |
