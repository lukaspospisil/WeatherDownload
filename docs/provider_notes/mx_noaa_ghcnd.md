# NOAA GHCN-Daily Mexico

<p align="right">
  <img src="../images/logo.svg" alt="WeatherDownload logo" width="180">
</p>

This note covers the thin `MX / ghcnd / daily` wrapper around the shared NOAA GHCN-Daily implementation. The shared GHCN wrapper behavior is documented in more detail in [NOAA GHCN-Daily Direct-Prefix Wrappers](ghcnd_direct_prefix_wrappers.md).
...
- the GHCN prefix `MX` matches the WeatherDownload country code directly
- only Mexican stations are exposed on this wrapper
...
`open_water_evaporation` is intentionally unsupported on this wrapper. The current inventory audit did not justify advertising raw `EVAP` for Mexico.
...
- `open_water_evaporation` is not advertised for `MX / ghcnd / daily`

## Examples

```powershell
weatherdownload stations elements --country MX --provider ghcnd --resolution daily --include-mapping
```
