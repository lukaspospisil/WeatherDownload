...
This note documents the current Latvian Environment, Geology and Meteorology Centre / `lvgmc` provider slices implemented in WeatherDownload.

## Provider identifiers

- country: `LV`
- provider: `lvgmc`
- resolution(s): `1hour`, `daily`
...
The factual archive resource `339f73e4-20cf-4cea-be65-dcfd4b3b742c` and the last-48-hours operational resource `17460efb-ae99-4d1d-8144-1068f184b05f` are not used by the currently implemented `1hour` or `daily` provider paths.
...
- `LV / lvgmc / 1hour` is therefore archive-capable within that recent window, but it is not a multi-decade historical hourly climate archive
...
- `LV / lvgmc / 1hour` is an official hourly downloader backed directly by the public LVGMC recent archive resource
- it is not built by provider-side aggregation from another WeatherDownload provider
- it uses the same hourly AVG/MIN/MAX/SUM archive resource that the daily implementation uses internally
- LVGMC documents `DATETIME` as the end of the represented UTC hour
- WeatherDownload therefore exposes hourly timestamps as the represented hour start:
  - `timestamp = DATETIME - 1 hour`
- the CKAN query interval is built as:
  - `DATETIME > <start_timestamp>`
  - `DATETIME <= <end_timestamp_plus_one_hour>`

That keeps hourly timestamps internally consistent with the daily UTC date bucketing rule below.
...
## Supported observed hourly elements

| Raw | Canonical | Source unit | WeatherDownload hourly semantics |
| --- | --- | --- | --- |
| `HTDRY` | `tas_mean` | `degC` | direct hourly archive value for the preceding UTC hour |
| `HPRAB` | `precipitation` | `mm` | direct hourly archive value for the preceding UTC hour |
| `HWNDS` | `wind_speed` | `m/s` | direct hourly archive value for the preceding UTC hour |
| `HWSMX` | `wind_speed_max` | `m/s` | direct hourly archive value for the preceding UTC hour |
| `HRLH` | `relative_humidity` | `%` | direct hourly archive value for the preceding UTC hour |
| `HPRSL` | `pressure` | `hPa` | direct hourly archive value for the preceding UTC hour |
| `HSNOW` | `snow_depth` | `cm` | direct hourly archive value for the preceding UTC hour |

## Supported observed and aggregated daily elements
...
- `tas_max` and `tas_min` are not exposed on `LV / lvgmc / 1hour`; although the archive contains hourly max/min style raw fields, this first hourly public slice stays conservative and exposes only the direct hourly fields above
...
- `wind_direction` is unsupported because this provider does not currently expose it as a WeatherDownload element on either the hourly or daily path
...
- `LV / lvgmc / 1hour` and `LV / lvgmc / daily` are not observed FAO-ready provider paths
