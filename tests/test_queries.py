...
from weatherdownload import ObservationQuery, QueryValidationError, list_providers, list_resolutions, list_supported_elements, normalize_provider_name, validate_observation_query


class DiscoveryTests(unittest.TestCase):
    def test_list_providers_contains_historical_csv(self) -> None:
        self.assertIn('historical_csv', list_providers())
    ...
    def test_list_supported_elements_can_return_mapping_table(self) -> None:
        mapping = list_supported_elements(resolution='daily', provider='historical_csv', include_mapping=True)
    ...
    def test_list_supported_elements_accepts_provider(self) -> None:
...
class ObservationQueryValidationTests(unittest.TestCase):
    def test_normalize_provider_name_accepts_provider(self) -> None:
        self.assertEqual(normalize_provider_name('historical_csv'), 'historical_csv')
        self.assertEqual(normalize_provider_name('ghcnd'), 'ghcnd')

    def test_query_normalizes_station_ids_and_translates_canonical_elements_for_cz(self) -> None:
        query = ObservationQuery(provider='historical_csv', resolution='daily', station_ids=[' 0-20000-0-11406 ', '0-20000-0-11406', '0-20000-0-11414'], start_date='2024-01-01', end_date='2024-12-31', elements=[' tas_max ', 'TMI', 'tas_max'])
    ...
    def test_query_rejects_missing_provider(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='US', resolution='daily', station_ids=['USC00000001'], start_date='2020-05-01', end_date='2020-05-02', elements=['tas_max'])
    ...
    def test_query_accepts_raw_provider_codes(self) -> None:
        query = ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start_date='2024-01-01', end_date='2024-01-02', elements=['tma', 'TMI'])
        self.assertEqual(query.elements, ['TMA', 'TMI'])

    def test_same_canonical_daily_request_shape_works_for_cz_and_de(self) -> None:
        cz_query = ObservationQuery(country='CZ', provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start_date='2024-01-01', end_date='2024-01-02', elements=['tas_mean', 'sunshine_duration'])
        de_query = ObservationQuery(country='DE', provider='historical', resolution='daily', station_ids=['00044'], start_date='2024-01-01', end_date='2024-01-02', elements=['tas_mean', 'sunshine_duration'])
    ...
    def test_valid_but_not_implemented_combination_is_still_query_valid(self) -> None:
        ...
        self.assertEqual(query.provider, 'now')
        self.assertEqual(query.resolution, '10min')

    def test_query_rejects_mixed_date_and_datetime_ranges(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start='2024-01-01T00:00:00Z', end='2024-01-31T00:00:00Z', start_date='2024-01-01', end_date='2024-01-31')

    def test_hourly_query_rejects_date_only_precision(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(provider='historical_csv', resolution='1hour', station_ids=['0-20000-0-11406'], start_date='2024-01-01', end_date='2024-01-02', elements=['vapour_pressure'])

    def test_daily_query_rejects_datetime_precision(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start='2024-01-01T00:00:00Z', end='2024-01-31T00:00:00Z')

    def test_de_daily_query_accepts_dwd_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='DE', provider='historical', resolution='daily', station_ids=['00003'], start_date='2024-01-01', end_date='2024-01-02', elements=['tmk', 'rsk'])
        canonical_query = ObservationQuery(country='DE', provider='historical', resolution='daily', station_ids=['00003'], start_date='2024-01-01', end_date='2024-01-02', elements=['tas_mean', 'precipitation'])
    ...
    def test_ca_daily_query_accepts_ghcnd_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='CA', provider='ghcnd', resolution='daily', station_ids=['CA000000001'], start_date='2020-06-01', end_date='2020-06-02', elements=['tavg', 'tmax', 'prcp', 'snwd'])
        canonical_query = ObservationQuery(country='CA', provider='ghcnd', resolution='daily', station_ids=['CA000000001'], start_date='2020-06-01', end_date='2020-06-02', elements=['tas_mean', 'tas_max', 'precipitation', 'snow_depth'])
    ...
    def test_cz_ghcnd_daily_query_accepts_ghcnd_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='CZ', provider='ghcnd', resolution='daily', station_ids=['EZM00011406'], start_date='2020-05-01', end_date='2020-05-02', elements=['tavg', 'tmax', 'prcp', 'snwd'])
        canonical_query = ObservationQuery(country='CZ', provider='ghcnd', resolution='daily', station_ids=['EZM00011406'], start_date='2020-05-01', end_date='2020-05-02', elements=['tas_mean', 'tas_max', 'precipitation', 'snow_depth'])
        self.assertEqual(raw_query.country, 'CZ')
        self.assertEqual(raw_query.elements, ['TAVG', 'TMAX', 'PRCP', 'SNWD'])
        self.assertEqual(canonical_query.elements, ['TAVG', 'TMAX', 'PRCP', 'SNWD'])

    def test_de_ghcnd_daily_query_accepts_ghcnd_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='DE', provider='ghcnd', resolution='daily', station_ids=['GM000000001'], start_date='2020-12-01', end_date='2020-12-02', elements=['tavg', 'tmax', 'prcp', 'snwd'])
    ...
    def test_be_tenmin_query_accepts_be_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='BE', provider='historical', resolution='10min', station_ids=['6414'], start='2024-01-01T00:10:00Z', end='2024-01-01T00:20:00Z', elements=['temp_dry_shelter_avg', 'pressure'])
        canonical_query = ObservationQuery(country='BE', provider='historical', resolution='10min', station_ids=['6414'], start='2024-01-01T00:10:00Z', end='2024-01-01T00:20:00Z', elements=['tas_mean', 'pressure'])
        self.assertEqual(raw_query.country, 'BE')
        self.assertEqual(raw_query.elements, ['temp_dry_shelter_avg', 'pressure'])
        self.assertEqual(canonical_query.elements, ['temp_dry_shelter_avg', 'pressure'])

    def test_dk_hourly_query_accepts_dk_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='DK', provider='historical', resolution='1hour', station_ids=['06180'], start='2024-01-01T01:00:00Z', end='2024-01-01T02:00:00Z', elements=['mean_temp', 'mean_pressure'])
    ...
    def test_dk_tenmin_query_accepts_dk_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='DK', provider='historical', resolution='10min', station_ids=['06180'], start='2024-01-01T00:10:00Z', end='2024-01-01T00:20:00Z', elements=['temp_dry', 'pressure'])
        canonical_query = ObservationQuery(country='DK', provider='historical', resolution='10min', station_ids=['06180'], start='2024-01-01T00:10:00Z', end='2024-01-01T00:20:00Z', elements=['tas_mean', 'pressure'])
        self.assertEqual(raw_query.country, 'DK')
        self.assertEqual(raw_query.elements, ['temp_dry', 'pressure'])
        self.assertEqual(canonical_query.elements, ['temp_dry', 'pressure'])

    def test_nl_daily_query_accepts_nl_elements_and_canonical_names(self) -> None:
        raw_query = ObservationQuery(country='NL', provider='knmi', resolution='daily', station_ids=['260'], start_date='2024-01-01', end_date='2024-01-02', elements=['TG', 'PG'])
        canonical_query = ObservationQuery(country='NL', provider='knmi', resolution='daily', station_ids=['260'], start_date='2024-01-01', end_date='2024-01-02', elements=['tas_mean', 'pressure'])
        self.assertEqual(raw_query.country, 'NL')
        self.assertEqual(raw_query.elements, ['TG', 'PG'])
        self.assertEqual(canonical_query.elements, ['TG', 'PG'])
    ...
    def test_query_rejects_unknown_canonical_element_for_path(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='DE', provider='historical', resolution='daily', station_ids=['00003'], start_date='2024-01-01', end_date='2024-01-02', elements=['tas_period_max'])

    def test_daily_query_accepts_explicit_all_history_mode(self) -> None:
        query = ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], all_history=True, elements=['tas_mean'])
    ...
    def test_subdaily_query_accepts_explicit_all_history_mode(self) -> None:
        query = ObservationQuery(country='DE', provider='historical', resolution='1hour', station_ids=['00044'], all_history=True, elements=['tas_mean'])
        self.assertTrue(query.all_history)
    ...
    def test_all_history_is_mutually_exclusive_with_explicit_ranges(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start_date='2024-01-01', end_date='2024-01-02', all_history=True)

    def test_query_requires_explicit_range_without_all_history(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'])

    def test_validate_observation_query_returns_query(self) -> None:
        query = ObservationQuery(provider='historical_csv', resolution='daily', station_ids=['0-20000-0-11406'], start_date=date(2024, 1, 1), end_date=date(2024, 12, 31))
