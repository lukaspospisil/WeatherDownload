...
SAMPLE_TENMIN_FILES_JSON = json.dumps(
    {
        'files': [
            {'filename': 'KMDS__OPER_P___10M_OBS_L2_202401010910.nc'},
            {'filename': 'KMDS__OPER_P___10M_OBS_L2_202401010920.nc'},
        ]
    }
)
...
class KnmiProviderTests(unittest.TestCase):
    ...
    def test_read_station_observation_metadata_country_nl_from_local_fixture(self) -> None:
        ...
        self.assertIn('10-minute air temperature', metadata['name'].tolist())
    ...
    def test_nl_tenmin_download_uses_near_real_time_dataset_and_normalizes_output(self) -> None:
        station_metadata = read_station_metadata(country='NL', source_url=str(SAMPLE_STATIONS_PATH))
        parsed_payloads = {
            'KMDS__OPER_P___10M_OBS_L2_202401010910.nc': {
                ...
                'variables': {
                    'ta': pd.Series([3.1, 4.2]),
                    'pp': pd.Series([1012.1, 1011.2]),
                },
            },
            'KMDS__OPER_P___10M_OBS_L2_202401010920.nc': {
                ...
                'variables': {
                    'ta': pd.Series([3.3, 4.4]),
                    'pp': pd.Series([1011.9, 1011.0]),
                },
            },
        }
        filename_order = iter(parsed_payloads.keys())
        ...
        self.assertTrue(observations['flag'].isna().all())
        self.assertTrue(observations['quality'].isna().all())
        self.assertEqual(observations.iloc[0]['timestamp'].isoformat(), '2024-01-01T09:10:00+00:00')
