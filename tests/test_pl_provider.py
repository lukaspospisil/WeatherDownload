...
class PolandProviderTests(unittest.TestCase):
    ...
    def test_pl_queries_accept_scope_specific_canonical_and_raw_codes(self) -> None:
        canonical_query = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2025-01-02', elements=['tas_mean', 'precipitation'])
        raw_query = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2025-01-02', elements=['STD', 'SMDB'])
        ...
        hourly_raw_query = ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start='2025-01-01T00:00:00Z', end='2025-01-01T01:00:00Z', elements=['TEMP', 'PPPS'])
    ...
    def test_pl_query_rejects_unsupported_resolution_and_scope_specific_element(self) -> None:
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start_date='2025-01-01', end_date='2025-01-01', elements=['tas_mean'])
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2025-01-02', elements=['wind_speed'])
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='PL', provider='historical_klimat', resolution='daily', station_ids=['00375'], start_date='2026-01-01', end_date='2026-01-02', elements=['sunshine_duration'])
        with self.assertRaises(QueryValidationError):
            ObservationQuery(country='PL', provider='historical_klimat', resolution='1hour', station_ids=['00375'], start='2025-01-01T00:00:00Z', end='2025-01-01T01:00:00Z', elements=['tas_mean'])
    ...
    def test_build_targets_country_pl_handles_daily_and_hourly_archive_shapes(self) -> None:
        query_recent = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2026-01-02', elements=['tas_mean'])
        ...
        hourly_recent = ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start='2025-01-01T00:00:00Z', end='2025-01-01T01:00:00Z', elements=['tas_mean'])
        ...
        query_legacy = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='1999-01-01', end_date='1999-01-02', elements=['tas_mean'])
        ...
        hourly_legacy = ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start='1999-01-01T00:00:00Z', end='1999-01-01T01:00:00Z', elements=['tas_mean'])
        ...
        klimat_recent = ObservationQuery(country='PL', provider='historical_klimat', resolution='daily', station_ids=['00375'], start_date='2026-01-01', end_date='2026-01-02', elements=['tas_mean'])
    ...
    def test_download_daily_observations_country_pl_combines_station_year_and_current_year_month(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2026-01-02', elements=['tas_mean', 'precipitation', 'sunshine_duration'])
    ...
    def test_download_hourly_observations_country_pl_station_year_archive(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start='2025-01-01T00:00:00Z', end='2025-01-01T01:00:00Z', elements=['tas_mean', 'pressure', 'wind_speed'])
    ...
    def test_download_daily_observations_country_pl_klimat_month_archive(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical_klimat', resolution='daily', station_ids=['00375'], start_date='2026-01-01', end_date='2026-01-02', elements=['tas_mean', 'tas_max', 'tas_min', 'precipitation'])
        with patch('weatherdownload.providers.pl.daily.requests.get', side_effect=fake_get):
            observations = download_observations(query, country='PL', station_metadata=station_metadata)
        self.assertEqual(list(observations.columns), PL_NORMALIZED_DAILY_COLUMNS)
        self.assertEqual(sorted(observations['element'].unique().tolist()), ['precipitation', 'tas_max', 'tas_mean', 'tas_min'])
        self.assertEqual(observations['provider'].unique().tolist(), ['historical_klimat'])
        value_lookup = observations.set_index(['element', 'observation_date'])['value']
        flag_lookup = observations.set_index(['element', 'observation_date'])['flag']
    ...
    def test_pl_daily_contract_mapping_and_key_values_are_stable(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical', resolution='daily', station_ids=['00375'], start_date='2025-01-01', end_date='2025-01-02', elements=list(EXPECTED_PL_DAILY_MAPPING.keys()))
        with patch('weatherdownload.providers.pl.daily.requests.get', side_effect=fake_get):
            observations = download_observations(query, country='PL', station_metadata=station_metadata)
    ...
    def test_pl_hourly_contract_mapping_and_key_values_are_stable(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical', resolution='1hour', station_ids=['00375'], start='2025-01-01T00:00:00Z', end='2025-01-01T01:00:00Z', elements=list(EXPECTED_PL_HOURLY_MAPPING.keys()))
        with patch('weatherdownload.providers.pl.hourly.requests.get', side_effect=fake_get):
            observations = download_observations(query, country='PL', station_metadata=station_metadata)
        mapping = {row.element: row.element_raw for row in observations[['element', 'element_raw']].drop_duplicates().itertuples(index=False)}
    ...
    def test_pl_klimat_contract_mapping_and_synop_behavior_remains_unchanged(self) -> None:
        ...
        query = ObservationQuery(country='PL', provider='historical_klimat', resolution='daily', station_ids=['00375'], start_date='2026-01-01', end_date='2026-01-02', elements=list(EXPECTED_PL_KLIMAT_MAPPING.keys()))
        with patch('weatherdownload.providers.pl.daily.requests.get', side_effect=fake_get):
            observations = download_observations(query, country='PL', station_metadata=station_metadata)
        mapping = {row.element: row.element_raw for row in observations[['element', 'element_raw']].drop_duplicates().itertuples(index=False)}
