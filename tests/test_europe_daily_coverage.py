...
class EuropeCoverageTests(unittest.TestCase):
    ...
    def test_classification_matches_registry_and_documented_overrides(self) -> None:
        ...
        self.assertEqual(summary['daily']['BA']['status'], 'ghcnd_daily')
        ...
        self.assertEqual(summary['daily']['ME']['providers'], ['ghcnd'])
        self.assertEqual(summary['daily']['MK']['status'], 'ghcnd_daily')
        self.assertEqual(summary['daily']['MK']['providers'], ['ghcnd'])
        ...
        self.assertEqual(summary['daily']['EE']['providers'], ['ilmateenistus'])
        self.assertEqual(summary['daily']['FI']['status'], 'national_daily')
        ...
        self.assertEqual(summary['daily']['IS']['status'], 'national_daily')
        self.assertEqual(summary['daily']['IS']['providers'], ['vedur'])
        ...
        self.assertEqual(summary['daily']['LV']['providers'], ['lvgmc'])
        ...
        self.assertEqual(summary['daily']['RS']['status'], 'ghcnd_daily')
    ...
    def test_generated_svg_renders_context_land_separately_from_coverage_status(self) -> None:
        ...
        self.assertIn('.daily-audit-outline { fill: none; stroke: #ef6c00; stroke-width: 1.4;', svg_text)
    ...
    def test_data_coverage_documentation_references_all_maps_and_non_fao_scope(self) -> None:
        ...
        self.assertIn('They do not imply that all variables are available in all countries or at all stations.', doc_text)
        self.assertIn('`BG / nimh / daily` is intentionally narrow and currently exposes only `precipitation` and `snow_depth`.', doc_text)
        ...
        self.assertIn('Orange outline means an official national daily-provider audit was attempted and failed', doc_text)
        self.assertIn('Orange - investigated with a research note, but no safe implemented daily provider exists', doc_text)
        self.assertIn('`GHCN-Daily` fallback coverage is distinct from official national provider coverage.', doc_text)
        self.assertIn('`Fallback + research note` means WeatherDownload can still download daily data there', doc_text)
        self.assertIn('Researched Unresolved Countries', doc_text)
        self.assertIn('[kv_ihmk_research.md](provider_notes/kv_ihmk_research.md)', doc_text)
        self.assertIn('[mc_monaco_research.md](provider_notes/mc_monaco_research.md)', doc_text)
        self.assertIn('[sm_san_marino_research.md](provider_notes/sm_san_marino_research.md)', doc_text)
        self.assertIn('[va_vatican_research.md](provider_notes/va_vatican_research.md)', doc_text)
        self.assertIn('GHCN-Daily Fallback With Failed Official Audit', doc_text)
        self.assertIn('[by_belhydromet_research.md](provider_notes/by_belhydromet_research.md)', doc_text)
        self.assertIn('[ua_ukrhydromet_research.md](provider_notes/ua_ukrhydromet_research.md)', doc_text)
    ...
    def test_readme_shows_daily_map_and_links_to_data_coverage_page(self) -> None:
        ...
        self.assertIn('fill', readme_text)
        self.assertIn('official national daily-provider audit was attempted and failed', readme_text)
        self.assertIn('shown only as geographic context', readme_text)
        self.assertIn('Orange fill means researched unresolved daily coverage', readme_text)
        self.assertIn('`GHCN-Daily` fallback coverage is distinct', readme_text)
