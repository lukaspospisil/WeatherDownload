...
class DownloadTenminExampleTests(unittest.TestCase):
    ...
    def test_main_uses_shared_ch_query_shape(self) -> None:
        sample = pd.DataFrame([{'station_id': 'AIG', 'gh_id': '0-20000-0-06712', 'element': 'tas_mean', 'element_raw': 'tre200s0', 'timestamp': '2025-12-31T23:50:00Z', 'value': -3.7, 'flag': None, 'quality': None, 'provider': 'historical', 'resolution': '10min'}])
        buffer = io.StringIO()
        with patch.object(download_tenmin, 'download_observations', return_value=sample) as download_mock:
    ...
    def test_main_uses_shared_dk_query_shape(self) -> None:
        sample = pd.DataFrame([{'station_id': '06180', 'gh_id': None, 'element': 'tas_mean', 'element_raw': 'temp_dry', 'timestamp': '2024-01-01T00:10:00Z', 'value': 2.1, 'flag': None, 'quality': None, 'provider': 'historical', 'resolution': '10min'}])
        buffer = io.StringIO()
        with patch.object(download_tenmin, 'download_observations', return_value=sample) as download_mock:
    ...
    def test_main_uses_shared_hu_query_shape(self) -> None:
        sample = pd.DataFrame([{'station_id': '13704', 'gh_id': None, 'element': 'tas_mean', 'element_raw': 'ta', 'timestamp': '2026-01-01T00:00:00Z', 'value': 1.4, 'flag': None, 'quality': None, 'provider': 'historical', 'resolution': '10min'}])
        buffer = io.StringIO()
        with patch.object(download_tenmin, 'download_observations', return_value=sample) as download_mock:
