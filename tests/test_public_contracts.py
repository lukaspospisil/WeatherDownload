...
SAMPLE_GEOSPHERE_TENMIN_CSV_PATH = Path('tests/data/sample_geosphere_klima_v2_10min.csv')
SAMPLE_GEOSPHERE_TENMIN_CSV_TEXT = SAMPLE_GEOSPHERE_TENMIN_CSV_PATH.read_text(encoding='utf-8')
...
def test_hourly_download_contract_is_stable_for_supported_belgium_path() -> None:
    expected_columns = ['station_id', 'gh_id', 'element', 'element_raw', 'timestamp', 'value', 'flag', 'quality', 'dataset_scope', 'resolution']
    observations = _download_hourly_fixture('BE')
    assert list(observations.columns) == expected_columns
    assert observations['element'].str.match(r'^[a-z0-9_]+$').all()
    assert observations['element_raw'].notna().all()
    assert observations['timestamp'].map(lambda value: hasattr(value, 'isoformat')).all()
...
def test_tenmin_download_contract_is_stable_for_supported_austria_path() -> None:
    expected_columns = ['station_id', 'gh_id', 'element', 'element_raw', 'timestamp', 'value', 'flag', 'quality', 'dataset_scope', 'resolution']
    observations = _download_tenmin_fixture('AT')
    assert list(observations.columns) == expected_columns
    assert observations['element'].str.match(r'^[a-z0-9_]+$').all()
    assert observations['element_raw'].notna().all()
    assert observations['timestamp'].map(lambda value: hasattr(value, 'isoformat')).all()
...
def test_download_fao_bundle_shape_is_stable_across_supported_fao_countries() -> None:
    ...
    for country in ['CZ', 'DE', 'AT', 'BE', 'DK', 'NL', 'SE']:
        ...
        series = [{'station_id': 'TEST1', 'full_name': 'Test Station', 'latitude': 50.0, 'longitude': 14.0, 'elevation_m': 250.0, 'date': ['2024-01-01', '2024-01-02'], 'tas_mean': [1.0, 2.0], 'tas_max': [3.0, 4.0], 'tas_min': [-1.0, 0.0], 'wind_speed': [pd.NA, pd.NA] if country == 'SE' else [2.5, 3.0], 'vapour_pressure': [pd.NA, pd.NA] if country in {'AT', 'BE', 'DK', 'NL', 'SE'} else [7.0, 8.0], 'sunshine_duration': [pd.NA, pd.NA] if country == 'SE' else [0.5, 0.8]}]
