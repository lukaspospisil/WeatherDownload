...
class NetherlandsKnmiProviderTests(unittest.TestCase):
    def test_supported_country_and_discovery_expose_knmi_and_ghcnd_daily(self) -> None:
    ...
    def test_knmi_query_accepts_canonical_and_raw_codes(self) -> None:
        canonical_query = ObservationQuery(
            ...
            end_date='2024-01-02',
            elements=['tas_mean', 'precipitation', 'wind_speed_max', 'solar_radiation'],
        )
        raw_query = ObservationQuery(
            ...
            start_date='2024-01-01',
            end_date='2024-01-02',
            elements=['TG', 'RH', 'FXX', 'Q'],
        )

        self.assertEqual(canonical_query.elements, ['TG', 'RH', 'FXX', 'Q'])
        self.assertEqual(raw_query.elements, ['TG', 'RH', 'FXX', 'Q'])
    ...
    def test_knmi_daily_download_uses_public_csv_and_normalizes_output(self) -> None:
        ...
        def fake_post(url, data=None, timeout=60):
            ...
            self.assertEqual(data['vars'], 'TG:RH:FXX:Q')
            self.assertEqual(data['fmt'], 'csv')
        ...
        self.assertEqual(sorted(observations['element'].unique().tolist()), ['precipitation', 'solar_radiation', 'tas_mean', 'wind_speed_max'])
        self.assertEqual(sorted(observations['element_raw'].unique().tolist()), ['FXX', 'Q', 'RH', 'TG'])
        ...
        self.assertAlmostEqual(float(lookup[('wind_speed_max', '2024-01-01')]), 9.8)
    ...
    def test_provider_note_and_capabilities_docs_are_updated(self) -> None:
        ...
        self.assertIn('wind_speed_max', provider_note)
        self.assertIn('fmt=csv', provider_note)
        self.assertIn('0.01', provider_note)
        self.assertIn('allow-derived', provider_note)
        self.assertIn(
            '| `NL` | `knmi` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `wind_speed`, `wind_speed_max`, `relative_humidity`, `pressure`, `sunshine_duration`, `solar_radiation` |'.replace('`', ''),
            capabilities.replace('`', ''),
        )
        self.assertIn(
            '| `NL` | `ghcnd` | `daily` | `tas_mean`, `tas_max`, `tas_min`, `precipitation`, `snow_depth` |'.replace('`', ''),
            capabilities.replace('`', ''),
        )
