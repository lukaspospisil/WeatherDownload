...
class BelgiumTenminCliTests(unittest.TestCase):
    def test_tenmin_cli_explicit_country_be_uses_be_query_shape(self) -> None:
        sample = pd.DataFrame([
            {
                'station_id': '6414',
                'gh_id': None,
        ...
        with patch('weatherdownload.cli.download_observations', return_value=sample) as download_mock:
            with redirect_stdout(buffer):
                exit_code = main([
                    'observations', '10min', '--country', 'BE', '--station-id', '6414', '--element', 'tas_mean', '--start', '2024-01-01T00:10:00Z', '--end', '2024-01-01T00:20:00Z'
                ])
